...
class Window(QtWidgets.QWidget):
        ...
        if self.markerOccurred:
            for i, command in enumerate(self.commandStrings):
                if self.currentMarker == command:
                    num_samples = int(self.commandDataConfigs[i].duration * self.sampleRate)
                    [self.y[0].popleft() for d in range(num_samples)] 
                    num = self.GeneratePseudoEMG(self.sampleRate,self.commandDataConfigs[i].duration, self.commandDataConfigs[i].noise_level, 
                                                    self.commandDataConfigs[i].amplitude, self.commandDataConfigs[i].frequency)
                    self.markerOccurred = False
        else:# send baseline
            num_samples = int(self.baselineConfig.duration * self.sampleRate)
            [self.y[0].popleft() for d in range(num_samples)] 
            num = self.GeneratePseudoEMG(self.sampleRate,self.baselineConfig.duration, self.baselineConfig.noise_level, 
    ...
    def GeneratePseudoEMG(self,samplingRate, duration, noise_level, amplitude, frequency):
        ...
        """
        # Define the channels (assumed to be 8 in this example)
        channels = 8
        num_samples = int(duration * samplingRate)
        # Initialize the EMG signal array
        emg_signal = np.zeros((channels, num_samples))
